...
Double_t offset=0;
  ...
  TH1F *hmod2;  
  TH1F *htimeInteg;
  TPaveText *pt252 = new TPaveText(0.55,0.25,0.85,0.4, "NDC");
...
int fit252() {
  ...
  pt252->AddText("run252 - source on top of nero");
  pt252->AddText("between top and bottom paddle");
  pt252->AddText("using only both paddles in coincidence");
  ...
  pt->AddText("run252 - source on top of nero");
  pt->AddText("between top and bottom paddle");
  pt->AddText("using only both paddles in coincidence");
...
int Compare252(){
  ...
  //  TF1* f11 = new TF1("f11","1/559.70068*exp(-x/559.70068)",0,3000);
  //  f11->SetLineColor(3);
  //  f11->Draw("same");
