...
Double_t offset=0;
  ...
  TH1F *hmod2;  
  TH1F *htimeInteg;
  TPaveText *pt253 = new TPaveText(0.55,0.25,0.85,0.4, "NDC");


int fit253() {

  readsource("run253/TDC/quadA_2.dat",108070);

  pt253->AddText("run253 - source inside nero (holder)");
  pt253->AddText("one paddle on top of NERO -> gammas pass polyeth.");
  pt253->AddText("using only bottom paddle for stop");
...
int show_histo_mod253() {
  ...
  hmod2->SetTitle("run253 - Percent of all det. neutrons vs. moderation time");
...
int Compare253(){
  ...
  TF1* f11 = new TF1("f11","1/926.94661*exp(-x/926.94661)",0,3000);
  f11->SetLineColor(3);
  f11->Draw("same");
...
int ShowYield(){
  ...
  TF1* fsim = new TF1("fsim","1-exp(-x/926.94661)",0,3000);
  fsim->SetLineColor(3);
