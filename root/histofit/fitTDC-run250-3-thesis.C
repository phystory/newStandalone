...
Double_t offset=0;
  ...
  TH1F *hmod2;  
  TH1F *htimeInteg;
  TPaveText *pt250 = new TPaveText(0.55,0.2,0.8,0.35, "NDC");
...
int fit250() {
  ...
  pt250->AddText("run250 - in center of NERO");
  pt250->AddText("bottom paddle covers beam line hole");
  pt250->AddText("using only bottom paddle for stop");
...
int ShowYield(){
  ...
  TF1* fsim = new TF1("fsim","1-exp(-x/463.47331*5)",0,600);
  fsim->SetLineColor(3);
