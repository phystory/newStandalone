...
int fit261() {
  ...
  TPaveText *textbox = new TPaveText(0.55,0.65,0.85,0.8, "NDC");
  textbox->AddText("run261 - source on top of nero");
  textbox->AddText("source between NERO and top paddle");
  textbox->AddText("using bottom(under NERO) and");
  textbox->AddText("top paddle(on NERO) for start");


//----read and draw spectrum----//
  readsource("run261/TDC/quadA_2.dat",344265);
  gPad->SetLogy(0); 
  gPad->Modified();
...
//----fit exponential function----//
  ...
  expfit->SetLineColor(2);
  hsource->Fit("expfit","","",hsource->GetMaximumBin(),1500);
