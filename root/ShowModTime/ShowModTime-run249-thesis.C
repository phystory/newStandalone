...
//      hhisto = new TH1F(title,title,120,0,600);
      hhisto = new TH1F("moderation time","moderation time",120,0,600);
      hhisto->GetXaxis()->SetTitle("time in micro sec");
      hhisto->SetTitle("Moderation time - run249");    
	...
	hhisto->Fill(xch*0.2,value/runsec);
	hhisto->SetBinError(xch*0.2, sqrt((hhisto->GetBinError(xch*0.2))**2+(sqrt(value)/runsec)**2));
      } else {
	file >> ch0 >> xch >> ch0 >> value;
      }
    }
...
int showrun(TString runname){
  ...
  TString path_time = "/TDC/quadA_2.dat";
  ...
  //  c1->cd(1);
  path = runname+path_time!multiplicity_7;
  readhisto(path,1);
  hhisto->Draw();
  ...
  /*  c1->cd(2);
  path = runname+path_time!multiplicity_2;
  readhisto(path,1);
  hhisto->Draw();
...
int go249(){
  ...
  readhisto("run249/TDC/quadA_2.dat",55367);
  gStyle->SetOptStat(1000001);
  hhisto->Draw();
  c1->SaveAs("run249/modtime_raw.jpg");
}
...
int Fit249(){
  ...
  expfit->SetParameter(0,0.1);
  expfit->SetParameter(1,0.004);
  expfit->SetParameter(2,70);
  expfit->SetLineColor(2);
  hhisto->Fit("expfit","","",80,390);
  c1->SaveAs("run249/modtime_fitted.jpg");
}


int compare(){

  TF1* expfit_integ = new TF1("expfit_integ","1-exp((-x)/[0])",0,600);
  expfit_integ->SetParameter(0,expfit->GetParameter(2));
