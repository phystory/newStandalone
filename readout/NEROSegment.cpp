...
void NEROSegment::initialize()
{
  ...
  fprintf(stdout, "NERO Readout Status: %d\n", Readout_NERO);   
	...
	m_pVMEQDC12->reset();
	m_pVMEQDC12->setPedestalCurrent(60);
	m_pVMEQDC12->setThreshold(0,0);
	m_pVMEQDC12->setThreshold(1,0);
 ...
 /******************************************************************/
  ...
  if(Readout_NERO) {
	...
	if (datapresent) { // Valid data present
	  ...
	  //cout << "TDC data present WITH neutron" << endl;
	} else {
	  *rBuf = nodata; ++rBuf;
  ...
  /* ------------------------START - buffer input for CRS--------------------*/
      ...
      if (datapresent) // Valid data present 
	{
	  ...
	  //cout << "QDC data present WITH neutron" << endl;
	  QDCdata=1;
	}
      else
	{
  ...
  /* -------------------------END -- buffer input for CRS--------------------*/
	...
	if (datapresent) { // Valid data present
	  m_pVMETDC02->readEvent(rBuf); // Read data to data stream 
	  //cout << "TDC data present no neutron" << endl;
      ...
      if (datapresent) // Valid data present 
	{
	  m_pVMEQDC12-> readEvent(rBuf); // Read data to data stream 
	  QDCdata=1;
	  //cout << "QDC data present no neutron" << endl;
	}
      else
	{
	  *rBuf = nodata; ++rBuf;
